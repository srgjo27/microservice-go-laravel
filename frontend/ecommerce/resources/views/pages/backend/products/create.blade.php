...
@section('content')
    <form id="form_input" class="form d-flex flex-column flex-lg-row"
        data-kt-redirect="{{ route('backend.products.index') }}" action="{{ route('backend.products.store') }}"
...
@push('custom-scripts')
    ...
    <script>
        $('select[name="category_id"]').on('change', function() {
            $('input[name="category_name"]').val($('select[name="category_id"] option:selected').text());
        });
